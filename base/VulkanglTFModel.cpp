...
void vkglTF::Model::drawNode(Node *node, VkCommandBuffer commandBuffer, uint32_t instanceCount, uint32_t renderFlags, VkPipelineLayout pipelineLayout, uint32_t bindImageSet)
{
	if (node->mesh) {
		for (Primitive* primitive : node->mesh->primitives) {
			bool skip = false;
	...
	for (auto& child : node->children) {
		drawNode(child, commandBuffer, instanceCount, renderFlags);
	}
}

void vkglTF::Model::draw(VkCommandBuffer commandBuffer, uint32_t instanceCount, uint32_t renderFlags, VkPipelineLayout pipelineLayout, uint32_t bindImageSet)
{
	if (!buffersBound) {
		const VkDeviceSize offsets[1] = {0};
		vkCmdBindVertexBuffers(commandBuffer, 0, 1, &vertices.buffer, offsets);
	...
	for (auto& node : nodes) {
		drawNode(node, commandBuffer, instanceCount, renderFlags, pipelineLayout, bindImageSet);
	}
}
